...
menuitem_t SoundMenu[] = {
    {2, "M_SFXVOL", M_SfxVol, 's'}, {-1, "", 0, '\0'}, {2, "M_MUSVOL", M_MusicVol, 'm'}, {-1, "", 0, '\0'}};
...
void M_QuickSave(void) {
    ...
    DEH_snprintf(tempstring, sizeof(tempstring), QSPROMPT, savegamestrings[quickSaveSlot]);
...
// M_DrawMainMenu
//
void M_DrawMainMenu(void) { V_DrawPatchDirect(94, 2, W_CacheLumpName(DEH_String("M_DOOM"), PU_CACHE)); }
...
void M_DrawEpisode(void) { V_DrawPatchDirect(54, 38, W_CacheLumpName(DEH_String("M_EPISOD"), PU_CACHE)); }
...
#define JOY_BUTTON_PRESSED(x) (JOY_BUTTON_MAPPED(x) && (ev->data1 & (1 << (x))) != 0)
    ...
    if (saveStringEnter) {
        ...
        default:
            ...
            // Vanilla has a bug where the shift key is ignored when
            // entering a savegame name. If vanilla_keyboard_mapping is on,
            // we want to emulate this bug by using ev->data1. But if it's
            // turned off, it implies the user doesn't care about Vanilla
            // emulation: instead, use ev->data3 which gives the
