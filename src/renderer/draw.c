...
void R_DrawColumn(void) {
    ...
    if ((unsigned)dc_x >= SCREENWIDTH || dc_yl < 0 || dc_yh >= SCREENHEIGHT)
        error("R_DrawColumn: %i to %i at %i", dc_yl, dc_yh, dc_x);
#endif
...
#ifdef RANGECHECK
    if ((unsigned)dc_x >= SCREENWIDTH || dc_yl < 0 || dc_yh >= SCREENHEIGHT) {
        error("R_DrawFuzzColumn: %i to %i at %i", dc_yl, dc_yh, dc_x);
    }
#endif
...
#ifdef RANGECHECK
    if ((unsigned)x >= SCREENWIDTH || dc_yl < 0 || dc_yh >= SCREENHEIGHT) {
        error("R_DrawColumn: %i to %i at %i", dc_yl, dc_yh, x);
    }
...
void R_DrawSpan(void) {
    ...
    if (ds_x2 < ds_x1 || ds_x1 < 0 || ds_x2 >= SCREENWIDTH || (unsigned)ds_y > SCREENHEIGHT) {
        error("R_DrawSpan: %i to %i at %i", ds_x1, ds_x2, ds_y);
    }
//	dscount++;
#endif
...
void R_FillBackScreen(void) {
    ...
    if (background_buffer == NULL) {
        background_buffer =
            Z_Malloc(SCREENWIDTH * (SCREENHEIGHT - SBARHEIGHT) * sizeof(*background_buffer), PU_STATIC, NULL);
    }
