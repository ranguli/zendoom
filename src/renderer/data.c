...
void R_GenerateLookup(int texnum) {
    ...
    for (x = 0; x < texture->width; x++) {
        ...
        // error ("R_GenerateLookup: column without a patch");
        ...
        if (patchcount[x] > 1) {
            ...
            if (texturecompositesize[texnum] > 0x10000 - texture->height) {
                error("R_GenerateLookup: texture %i is >64k", texnum);
            }
...
void R_InitTextures(void) {
    ...
    for (i = 0; i < numtextures; i++, directory++) {
        ...
        texture = textures[i] = Z_Malloc(
            sizeof(texture_t) + sizeof(texpatch_t) * (SHORT(mtexture->patchcount) - 1), PU_STATIC, 0);
        ...
        for (j = 0; j < texture->patchcount; j++, mpatch++, patch++) {
            ...
            if (patch->patch == -1) {
                error("R_InitTextures: Missing patch in texture %s", texture->name);
            }
        }
...
int R_FlatNumForName(const char *name) {
    ...
    if (i == -1) {
        namet[8] = 0;
        memcpy(namet, name, 8);
        error("R_FlatNumForName: %s not found", namet);
    }
    return i - firstflat;
}
...
int R_TextureNumForName(const char *name) {
    ...
    if (i == -1) {
        error("R_TextureNumForName: %s not found", name);
    }
    return i;
}
