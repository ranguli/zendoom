...
void I_SetGrabMouseCallback(grabmouse_callback_t func) { grabmouse_callback = func; }
...
static void HandleWindowEvent(SDL_WindowEvent *event) {
    ...
    case SDL_WINDOWEVENT_FOCUS_LOST:
        ...
        // We want to save the user's preferred monitor to use for running
        // the game, so that next time we're run we start on the same
        // display. So every time the window is moved, find which display
        // we're now on and update the video_display config variable.
...
static void LimitTextureSize(int *w_upscale, int *h_upscale) {
    ...
    if (SDL_GetRendererInfo(renderer, &rinfo) != 0) {
        error("CreateUpscaledTexture: SDL_GetRendererInfo() call failed: %s", SDL_GetError());
    }
    ...
    if (*w_upscale != orig_w || *h_upscale != orig_h) {
        printf("CreateUpscaledTexture: Limited texture size to %dx%d "
               ...
               *w_upscale * SCREENWIDTH, *h_upscale * SCREENHEIGHT, max_scaling_buffer_pixels,
               rinfo.max_texture_width, rinfo.max_texture_height);
    }
}
...
static void CreateUpscaledTexture(boolean force) {
    ...
    if (SDL_GetRendererOutputSize(renderer, &w, &h) != 0) {
        error("Failed to get renderer output size: %s", SDL_GetError());
    }
    ...
    new_texture = SDL_CreateTexture(renderer, pixel_format, SDL_TEXTUREACCESS_TARGET, w_upscale * SCREENWIDTH,
...
/** Checks and handles all command-line parameters pertaining to graphics
 * settings
 * \ingroup cli
 **/
...
static void SetVideoMode(void) {
    ...
    if (screen == NULL) {
        ...
        if (screen == NULL) {
            error("Error creating window for video startup: %s", SDL_GetError());
        }
    ...
    if (SDL_GetCurrentDisplayMode(video_display, &mode) != 0) {
        error("Could not get display mode for video display #%d: %s", video_display, SDL_GetError());
    }
    ...
    if (renderer == NULL) {
        error("Error creating renderer for screen window: %s", SDL_GetError());
    }
