...
static byte *AutoAllocMemory(int *size, int default_ram, int min_ram) {
    ...
    while (zonemem == NULL) {
        ...
        if (default_ram < min_ram) {
            error("Unable to allocate %i MiB of RAM for zone", default_ram);
        }
...
void error(const char *error, ...) {
    ...
    if (already_quitting) {
        fprintf(stderr, "Warning: recursive call to error detected.\n");
        exit(-1);
    } else {
...
void *I_Realloc(void *ptr, size_t size) {
    ...
    if (size != 0 && new_ptr == NULL) {
        error("I_Realloc: failed on reallocation of %" PRIuPTR " bytes", size);
    }
