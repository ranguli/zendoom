        ...
        if (playeringame[i]) {
            ...
            if (netgame && !netdemo && !(gametic % ticdup)) {
                if (gametic > BACKUPTICS && consistancy[i][buf] != cmd->consistancy) {
                    error("consistency failure (%i should be %i)", cmd->consistancy, consistancy[i][buf]);
                }
                if (players[i].mo)
...
boolean G_CheckSpot(int playernum, mapthing_t *mthing) {
...

    {
        ...
        case 3072:
            ...
            break;
        default:
            error("G_CheckSpot: unexpected angle %d\n", an);
            xa = ya = 0;
            break;
        }
...
void G_DeathMatchSpawnPlayer(int playernum) {
    ...
    if (selections < 4)
        error("Only %i deathmatch spots, 4 required", selections);
...
void G_DoLoadGame(void) {
    ...
    if (save_stream == NULL) {
        error("Could not load savegame %s", savename);
    }
...
void G_DoSaveGame(void) {
    ...
    if (save_stream == NULL) {
        ...
        // to be nice, save to somewhere else before we call error().
...
void G_DoPlayDemo(void) {
    ...
    if (D_NonVanillaPlayback(demoversion == DOOM_191_VERSION, lumpnum, "Doom 1.91 demo format")) {
        ...
        error(message, demoversion, DOOM_VERSION, DemoVersionDescription(demoversion));
    }
...
boolean G_CheckDemoStatus(void) {
    ...
    if (timingdemo) {
        ...
        error("timed %i gametics in %i realtics (%f fps)", gametic, realtics, fps);
    }
    ...
    if (demorecording) {
        *demo_p++ = DEMOMARKER;
        ...
        Z_Free(demobuffer);
        demorecording = false;
        error("Demo %s recorded", demoname);
    }
