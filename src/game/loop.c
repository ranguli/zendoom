...
static int GetAdjustedTime(void) {
    ...
    if (new_sync) {
        // Use the adjustments from client.c only if we are
...
static boolean BuildNewTic(void) {
    ...
    if (new_sync) {
        ...
        if (!client_connected && maketic - gameticdiv > 2)
            return false;
...
static void D_Disconnected(void) {
    ...
    if (drone) {
        error("Disconnected from server in drone mode.");
    }
...
void D_StartNetGame(net_gamesettings_t *settings, netgame_startup_callback_t callback) {
    ...
    if (ticdup < 1) {
        error("D_StartNetGame: invalid ticdup value (%d)", ticdup);
    }
...
boolean D_InitNetGame(net_connect_data_t *connect_data) {
    ...
    if (M_CheckParm("-server") > 0 || M_CheckParm("-privateserver") > 0) {
        ...
        if (i > 0) {
            ...
            if (addr == NULL) {
                error("No server found on local LAN");
            }
        }
