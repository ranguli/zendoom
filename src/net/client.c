...
static clientstate_t client_state;
...
char *client_reject_reason = NULL;
...
boolean client_received_wait_data;
net_waitdata_t client_wait_data;
...
#define KD 0.02
    ...
    offsetms =
        KP * (FRACUNIT * error) - KI * (FRACUNIT * cumul_error) + (KD * FRACUNIT) * (last_error - error);
...
static void SetRejectReason(const char *s) {
    free(client_reject_reason);
    if (s != NULL) {
        client_reject_reason = strdup(s);
    } else {
...
static void NET_CL_ParseWaitingData(net_packet_t *packet) {
    ...
    memcpy(&client_wait_data, &wait_data, sizeof(net_waitdata_t));
    client_received_wait_data = true;
}
...
static void NET_CL_ParseLaunch(net_packet_t *packet) {
    ...
    client_wait_data.num_players = num_players;
...
boolean NET_CL_Connect(net_addr_t *addr, net_connect_data_t *data) {
    ...
    client_received_wait_data = false;
