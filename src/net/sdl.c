...
static void NET_SDL_FreeAddress(net_addr_t *addr) {
    ...
    error("NET_SDL_FreeAddress: Attempted to remove an unused address!");
}
...
static boolean NET_SDL_InitClient(void) {
    ...
    if (udpsocket == NULL) {
        error("NET_SDL_InitClient: Unable to open a socket!");
    }
...
static boolean NET_SDL_InitServer(void) {
    ...
    if (udpsocket == NULL) {
        error("NET_SDL_InitServer: Unable to bind to port %i", port);
    }
...
static void NET_SDL_SendPacket(net_addr_t *addr, net_packet_t *packet) {
    ...
    if (!SDLNet_UDP_Send(udpsocket, -1, &sdl_packet)) {
        error("NET_SDL_SendPacket: Error transmitting packet: %s", SDLNet_GetError());
    }
}
...
static boolean NET_SDL_RecvPacket(net_addr_t **addr, net_packet_t **packet) {
    ...
    if (result < 0) {
        error("NET_SDL_RecvPacket: Error receiving packet: %s", SDLNet_GetError());
    }
    ...
    *packet = net_new_packet(recvpacket->len);
...
void NET_SDL_AddrToString(net_addr_t *addr, char *buffer, int buffer_len) {
    ...
    M_snprintf(buffer, buffer_len, "%i.%i.%i.%i", (host >> 24) & 0xff, (host >> 16) & 0xff,
