 ...
 * Network server code
 *
 * This file contains server-side networking code for starting and stopping
 * servers, connecting or disconnecting players, broadcasting messages, etc
 */
...
static net_servestate_t servestate;
static boolean server_initialized = false;
static client_t clients[MAXNETNODES];
static client_t *sv_players[NET_MAXPLAYERS];
...
static client_recv_t recvwindow[BACKUPTICS][NET_MAXPLAYERS];

#define net_server_ExpandTicNum(b) NET_ExpandTicNum(recvwindow_start, (b))

static void net_server_disconnect_client(client_t *client) {
    if (client->active) {
        NET_Conn_Disconnect(&client->connection);
    }
}

/** Check that the client is properly connected: ie. not in the process of connecting or disconnecting **/
static boolean net_server_client_connected(client_t *client) {
...
static void net_server_SendConsoleMessage(client_t *client, const char *s, ...) PRINTF_ATTR(2, 3);
static void net_server_SendConsoleMessage(client_t *client, const char *s, ...) {
    char buf[1024];
    va_list args;
    net_packet_t *packet;
...
static void net_server_BroadcastMessage(const char *s, ...) PRINTF_ATTR(1, 2);
static void net_server_BroadcastMessage(const char *s, ...) {
    char buf[1024];
    va_list args;
    int i;
    ...
    for (i = 0; i < MAXNETNODES; ++i) {
        if (net_server_client_connected(&clients[i])) {
            net_server_SendConsoleMessage(&clients[i], "%s", buf);
        }
    }
...
static void net_server_AssignPlayers(void) {
    int i;
    int pl;
...
static int net_server_NumPlayers(void) {
    int i;
    int result;
    ...
    for (i = 0; i < NET_MAXPLAYERS; ++i) {
        if (sv_players[i] != NULL && net_server_client_connected(sv_players[i])) {
            result += 1;
        }
    }
...
static int net_server_NumReadyPlayers(void) {
    int result = 0;
    int i;

    for (i = 0; i < MAXNETNODES; ++i) {
        if (net_server_client_connected(&clients[i]) && !clients[i].drone && clients[i].ready) {
            ++result;
        }
    }
...
static int net_server_MaxPlayers(void) {
    int i;
...
static int net_server_NumDrones(void) {
    int i;
    int result;
    ...
    for (i = 0; i < MAXNETNODES; ++i) {
        if (net_server_client_connected(&clients[i]) && clients[i].drone) {
            result += 1;
        }
    }
...
static int net_server_NumClients(void) {
    int count;
    int i;
...
static client_t *net_server_Controller(void) {
    client_t *best;
    int i;
...
static void net_server_SendWaitingData(client_t *client) {
    net_waitdata_t wait_data;
    net_packet_t *packet;
    client_t *controller;
    int i;

    net_server_AssignPlayers();

    controller = net_server_Controller();

    wait_data.num_players = net_server_NumPlayers();
    wait_data.num_drones = net_server_NumDrones();
    wait_data.ready_players = net_server_NumReadyPlayers();
    wait_data.max_players = net_server_MaxPlayers();
...
static unsigned int net_server_LatestAcknowledged(void) {
...
static void net_server_AdvanceWindow(void) {
    unsigned int lowtic;
    int i;

    if (net_server_NumPlayers() <= 0) {
        return;
    }

    lowtic = net_server_LatestAcknowledged();
...
static client_t *net_server_find_client(net_addr_t *addr) {
    int i;
...
static void net_server_SendReject(net_addr_t *addr, const char *msg) {
    net_packet_t *packet;
...
static void net_server_InitNewClient(client_t *client, net_addr_t *addr, net_protocol_t protocol) {
    client->active = true;
...
static void net_server_ParseSYN(net_packet_t *packet, client_t *client, net_addr_t *addr) {
    unsigned int magic;
    ...
    if (protocol == NET_PROTOCOL_UNKNOWN) {
        ...
        M_snprintf(reject_msg, sizeof(reject_msg),
                   ...
                   "negotiated.",
                   client_version);
        net_server_SendReject(addr, reject_msg);
    ...
    if (servestate != SERVER_WAITING_LAUNCH) {
        NET_Log("server: error: not in waiting launch state, servestate=%d", servestate);
        net_server_SendReject(addr, "Server is not currently accepting connections");
        return;
    }

    // Before accepting a new client, check that there is a slot free.
    net_server_AssignPlayers();
    num_players = net_server_NumPlayers();

    if ((!data.drone && num_players >= net_server_MaxPlayers()) || net_server_NumClients() >= MAXNETNODES) {
        NET_Log("server: no more players, num_players=%d, max=%d", num_players, net_server_MaxPlayers());
        net_server_SendReject(addr, "Server is full!");
        return;
    }
    ...
    if (data.gamemode != sv_gamemode || data.gamemission != sv_gamemission) {
        char msg[128];
        NET_Log("server: wrong mode/mission, %d != %d || %d != %d", data.gamemode, sv_gamemode,
                data.gamemission, sv_gamemission);
        ...
        net_server_SendReject(addr, msg);
        return;
    }
    ...
    net_server_InitNewClient(client, addr, protocol);
...
static void net_server_ParseLaunch(client_t *client) {
    ...
    if (client != net_server_Controller()) {
        NET_Log("server: error: this client isn't the controller, %d != %d", client, net_server_Controller());
        return;
    }
    ...
    if (servestate != SERVER_WAITING_LAUNCH) {
        NET_Log("server: error: not in waiting launch state, state=%d", servestate);
        return;
    }
    ...
    servestate = SERVER_WAITING_START;
}
...
static void StartGame(void) {
    ...
    sv_settings.num_players = net_server_NumPlayers();
...
static boolean AllNodesReady(void) {
    ...
    for (i = 0; i < MAXNETNODES; ++i) {
        if (net_server_client_connected(&clients[i]) && !clients[i].ready) {
            return false;
        }
    }
...
static void SendAllWaitingData(void) {
    ...
    for (i = 0; i < MAXNETNODES; ++i) {
        if (net_server_client_connected(&clients[i]) && clients[i].ready) {
            net_server_SendWaitingData(&clients[i]);
        }
    }
}
...
static void net_server_ParseGameStart(net_packet_t *packet, client_t *client) {
    ...
    if (servestate != SERVER_WAITING_START) {
        NET_Log("server: error: not in waiting start state, servestate=%d", servestate);
        return;
    }
...
static void net_server_SendResendRequest(client_t *client, int start, int end) {
    net_packet_t *packet;
    client_recv_t *recvobj;
    int i;
...
static void net_server_CheckResends(client_t *client) {
    int i;
    int player;
...
static void net_server_ParseGameData(net_packet_t *packet, client_t *client) {
    client_recv_t *recvobj;
    unsigned int seq;
    unsigned int ackseq;
    ...
    if (servestate != SERVER_IN_GAME) {
        NET_Log("server: error: not in game state: servestate=%d", servestate);
        return;
    }
    ...
    ackseq = net_server_ExpandTicNum(ackseq);
    seq = net_server_ExpandTicNum(seq);
...
static void net_server_ParseGameDataACK(net_packet_t *packet, client_t *client) {
    unsigned int ackseq;
...
static void net_server_SendTics(client_t *client, unsigned int start, unsigned int end) {
    net_packet_t *packet;
    unsigned int i;
    ...
    for (i = start; i <= end; ++i) {
        ...
        if (i != cmd->seq) {
            error("Wanted to send %i, but %i is in its place", i, cmd->seq);
        }
...
static void net_server_ParseResendRequest(net_packet_t *packet, client_t *client) {
    ...
    net_server_SendTics(client, start, last);
}

// Send a response back to the client

void net_server_SendQueryResponse(net_addr_t *addr) {
    net_packet_t *reply;
    ...
    querydata.servestate = servestate;

    // Number of players/maximum players

    querydata.num_players = net_server_NumPlayers();
    querydata.max_players = net_server_MaxPlayers();
...
static void net_server_ParseHolePunch(net_packet_t *packet) {
...
static void net_server_MasterPacket(net_packet_t *packet) {
    ...
    case NET_MASTER_PACKET_TYPE_NAT_HOLE_PUNCH:
        net_server_ParseHolePunch(packet);
        break;
    }
}
...
static void net_server_Packet(net_packet_t *packet, net_addr_t *addr) {
    client_t *client;
    unsigned int packet_type;
    ...
    if (addr != NULL && addr == master_server) {
        net_server_MasterPacket(packet);
        return;
    }

    // Find which client this packet came from

    client = net_server_find_client(addr);
    ...
    if (packet_type == NET_PACKET_TYPE_SYN) {
        net_server_ParseSYN(packet, client, addr);
    } else if (packet_type == NET_PACKET_TYPE_QUERY) {
        net_server_SendQueryResponse(addr);
    } else if (client == NULL) {
        ...
        case NET_PACKET_TYPE_GAMESTART:
            net_server_ParseGameStart(packet, client);
            break;
        case NET_PACKET_TYPE_LAUNCH:
            net_server_ParseLaunch(client);
            break;
        case NET_PACKET_TYPE_GAMEDATA:
            net_server_ParseGameData(packet, client);
            break;
        case NET_PACKET_TYPE_GAMEDATA_ACK:
            net_server_ParseGameDataACK(packet, client);
            break;
        case NET_PACKET_TYPE_GAMEDATA_RESEND:
            net_server_ParseResendRequest(packet, client);
            break;
        default:
            // unknown packet type
...
static void net_server_PumpSendQueue(client_t *client) {
    net_full_ticcmd_t cmd;
    int recv_index;
    int num_players;
    int i;
    ...
    if (client->sendseq - net_server_LatestAcknowledged() > 40) {
        return;
    }
    ...
    net_server_SendTics(client, starttic, endtic);
...
void net_server_CheckDeadlock(client_t *client) {
    int nowtime;
    ...
    if (nowtime - client->last_gamedata_time > 1000) {
        ...
        for (i = 0; i < BACKUPTICS; ++i) {
            if (!recvwindow[client->player_number][i].active) {
                ...
                net_server_SendResendRequest(client, recvwindow_start + i, recvwindow_start + i + 5);
        ...
        if (i < BACKUPTICS && client->sendseq > client->acknowledged) {
            NET_Log("server: also resending tics %d-%d to break deadlock", client->acknowledged,
                    client->sendseq - 1);
            net_server_SendTics(client, client->acknowledged, client->sendseq - 1);
        }
    }
}
...
static void net_server_GameEnded(void) {
    int i;

    servestate = SERVER_WAITING_LAUNCH;
    sv_gamemode = indetermined;

    for (i = 0; i < MAXNETNODES; ++i) {
        if (clients[i].active) {
            net_server_disconnect_client(&clients[i]);
        }
    }
}
...
static void net_server_runClient(client_t *client) {
    // Run common code
    ...
    if (client->connection.state == NET_CONN_STATE_DISCONNECTED &&
        ...
        net_server_BroadcastMessage("Client '%s' timed out and disconnected", client->name);
    }
    ...
    if (!net_server_client_connected(client)) {
    ...
    if (servestate == SERVER_WAITING_LAUNCH) {
        ...
        if (client->last_send_time < 0 || I_GetTimeMS() - client->last_send_time > 1000) {
            net_server_SendWaitingData(client);
            client->last_send_time = I_GetTimeMS();
        }
    }

    if (servestate == SERVER_IN_GAME) {
        net_server_PumpSendQueue(client);
        net_server_CheckDeadlock(client);
    }
}
...
void net_server_run(void) {
    ...
    for (i = 0; i < MAXNETNODES; ++i) {
        if (clients[i].active) {
            net_server_runClient(&clients[i]);
        }
    }
    ...
    case SERVER_IN_GAME:
        net_server_AdvanceWindow();

        for (i = 0; i < NET_MAXPLAYERS; ++i) {
            if (sv_players[i] != NULL && net_server_client_connected(sv_players[i])) {
                net_server_CheckResends(sv_players[i]);
            }
        }
        break;
    }
...
void net_server_init(void) {
    ...
    // This will register servers with the
    // Chocolate Doom server, we don't want that.
    net_server_register_with_master();
